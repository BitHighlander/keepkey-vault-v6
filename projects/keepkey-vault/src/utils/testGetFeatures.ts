...
export async function testGetFeatures() {
    console.log('🧪 Testing get_features performance...\n');
    
    try {
        // First, get connected devices
        const devices = await invoke('get_connected_devices') as any[];
        ...
        for (let i = 0; i < 10; i++) {
            ...
            try {
                const features = await invoke('get_features', { deviceId }) as any;
...
if (typeof window !== 'undefined') {
    (window as any).testGetFeatures = testGetFeatures;
}