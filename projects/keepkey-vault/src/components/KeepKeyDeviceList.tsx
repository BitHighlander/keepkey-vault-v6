...
import { FaUsb, FaDownload, FaWallet, FaShieldAlt, FaExclamationTriangle, FaTools, FaTrash, FaCheckCircle, FaSync } from 'react-icons/fa'
...
export const KeepKeyDeviceList = ({ 
  ...
  const loadDevices = async () => {
    ...
    try {
      ...
      console.log(tag, 'Connected devices:', connectedDevices)
      
      // Map the devices to our format and fetch features and status for each
      const mappedDevices: Device[] = await Promise.all(
        connectedDevices
          .map(async (deviceData) => {
            const device: Device = {
              id: deviceData.device_id,
              name: deviceData.name || 'KeepKey Device',
              features: undefined,
              status: undefined
            }
            
            // Try to get features for this device
            try {
              const features = await invoke<DeviceFeatures>('get_features', { 
                deviceId: device.id 
              })
              device.features = features
              console.log(tag, `Got features for device ${device.id}:`, features)
            } catch (error) {
              console.warn(`Failed to get features for device ${device.id}:`, error)
              // Device is detected but not communicating - this is expected for some devices
            }
            ...
            if (device.features) {
              ...
              console.warn(`Device ${device.id} detected but no features available - communication failed`)
            }
